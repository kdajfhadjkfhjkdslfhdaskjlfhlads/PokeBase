...
import { _ as policyEnvKeys, a as envKeysForSelection, b as toEntries, d as servicesForEnvKeys, f as NEON_ENV_VAR_KEYS, g as fetchEnvKeys, h as credentialName, m as credentialEnvKeys, p as createApiFromOptions, r as ENV_PULL_SERVICES, u as serviceForEnvKey, v as previewCredentialScopes, y as resolveBranchPolicy } from "./env_services-Tz9G4JeT.js";
...
import { ErrorCode as ErrorCode$1, createNeonApiFromOptions as createNeonApiFromOptions$1, isPlatformError, loadConfigFromFile } from "@neon/config";
...
const resolveNeonEnvVars = async (ctx) => {
	...
	if (ctx.projectId && ctx.branchId) {
		...
		if (!ctx.implyAiGateway) return await fetchAndProject(pulled.config, ctx);
...
const resolveWithImpliedGateway = async (config, ctx, branch) => {
	...
	if (unreachable === null) return await fetchAndProject(withAiGateway(config), ctx);
...
const resolveSelectedServices = async (ctx, directServices, envKeys) => {
	...
	const [auth, dataApiEnabled, buckets, aiGatewayAvailable] = await Promise.all([
...
const readAiGatewayAvailable = async (api, projectId, branchId) => {
	try {
		...
		if (isPlatformError(error) && error.code === ErrorCode$1.FeatureUnavailable && (error.details.status === 404 || error.details.status === 501)) return false;
		throw error;
	}
};
...
const loadNeonConfig = async (cwd) => {
	try {
		...
		if (looksLikeMissingDependency(err)) throw new Error(`Could not load neon.ts: a package it imports is not installed. Run \`${formatInstallCommand(resolvePackageManager(cwd))}\`, then try again.\nOriginal error: ${message}`);
		throw err;
	}
};
//#endregion
